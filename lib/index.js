...
export const req = (keys, depiction) => {
  ...
  return {
    ...
    validator: (value, name) => {
      ...
      const objectValidations = results.validations.map(v => ({
        ...v,
        dPath: ["req", ...v.dPath]
      }));
      const reqValidations = keys
        .map(k => ({ key: k, success: Object.keys(value).includes(k) }))
      ...
      return {
        ...
        validations: [...objectValidations, ...reqValidations]
      };
    }
  };
};
